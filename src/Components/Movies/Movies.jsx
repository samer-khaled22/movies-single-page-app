...
function Movies(props) {
  ...
  async function getMovies(){
    if(searchTerm===""){
     let {data} = await axios (`https://api.themoviedb.org/3/trending/movie/${trending}?api_key=fd8809b2b50ab2e7a7dc7c9937cbc68d&page=${pageNum}`);
    setmovies(data.results)
    }else{
    let {data} = await axios (`https://api.themoviedb.org/3/search/movie?api_key=fd8809b2b50ab2e7a7dc7c9937cbc68d&query=${searchTerm}`);
    setmovies(data.results)
    }
    }
 ...
 return (
    ...
    <h2 className={style.topSearch}>Searching for a movie?</h2><br />
...
<div className="col-md-12 mb-3">
    ...
    <h2 className={`${style.header} `}>Trending  Movies  {trendHeader} </h2> 
    <span className={style.trendSpan}>Or Check</span>
    <button onClick={changeTrend} className="btn btn-outline-info my-3">{trendBtn}</button><br />
    </div>
</div>
{movies.map((value,index)=>{
return(
    <div key={index} className="col-md-3 mb-4">
    <Link to={{pathname: `/moviedetails`,state: {type:"movie"}}} onClick={()=>props.getItem(value)} >
    ...
    <p className="text-center">{value.overview.split(" " , 25).join(" ")}...</p></div>
...
<div className="d-flex justify-content-center pb-5">
    {firstPage}
    <button onClick={moreMovies} className="btn btn-outline-info px-5">Next Page</button>
</div>
</div>
    </>
    )
...
export default connect(mapStateToProps,{getItem})(Movies)
